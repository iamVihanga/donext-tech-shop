...
import { useUpdateRule } from "../actions/use-update-rules";
import { updateCompatibilityRuleSchema } from "../schemas/rules";
import { CompatibilityRule } from "./rules-table/columns";

type FormValues = z.infer<typeof updateCompatibilityRuleSchema>;
...
export function RuleEditForm({ rule, open, onOpenChange }: RuleEditFormProps) {
  const updateMutation = useUpdateRule();

  const form = useForm<FormValues>({
    resolver: zodResolver(updateCompatibilityRuleSchema),
    defaultValues: {
      name: "",
      description: "",
      ruleType: "",
  ...
  useEffect(() => {
    if (rule) {
      form.reset({
        ...
        primaryComponent: rule.primaryComponent,
        secondaryComponent: rule.secondaryComponent || undefined,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh]">
        <DialogHeader>
          <DialogTitle>Edit Compatibility Rule</DialogTitle>
          <DialogDescription>
            Update the compatibility rule configuration
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
